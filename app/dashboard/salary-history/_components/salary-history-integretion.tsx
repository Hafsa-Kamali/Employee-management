...
import SalaryHistoryTableDesktop from "./salary-history-table-desktop";
import SalaryHistoryTableMobile from "./salary-history-table-mobile";
...
const SalaryHistoryTable: React.FC<SalaryHistoryProps> = ({ walletAddress }) => {
  ...
  const employeeSalaryData = salaryData.filter((record) => record.receiverAddress === walletAddress);
  ...
  if (employeeSalaryData.length === 0) {
    return (
      <div className="w-full px-4 md:p-8 text-center text-white/50">No salary history available for this employee.</div>
    );
  }
